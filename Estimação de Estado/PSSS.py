        ...
        if i < (n_bus-1):
            ...
            if actual_bus == bus:
                ...
                for j in bus_looking:
                    ...
                    tetaj = state_vector[j - 1] if j > 0 else 0
                    tetaij = tetai - tetaj
                    aux_p += vj*(Bij*np.cos(tetaij) + Gij*np.sin(tetaij))
...
def read_meansured(filepath: str, filename: str, data: pd.core.frame.DataFrame = None) -> (dict, np.ndarray, list):
    if data is None:
    ...
    for i, line in enumerate(meansured.iterrows()):
        ...
        R[i,i] = meansured_line['Desvio_Pad']**2
        med_caracteristic = (meansured_line['Valor'], meansured_line['Desvio_Pad'])
...
def mount_y_bar(filepath: str, filename: str) -> (np.ndarray, dict):
    path_file = os.path.join(filepath, filename)
...
def linear_state_estimation(order_meansured: list, y_bar_matriz: np.ndarray) -> (np.ndarray, np.ndarray):
...
def identify_critical(E: np.ndarray, med_data: pd.core.frame.DataFrame):
    ...
    med_data.loc[med_data.index.isin(number_cmeans), 'Cricality'] = 'Critical Meansured'
    critical_sets = []
    csets = []
    amount_csets = 0
    for i in range(0, amount_meansured):
        ...
        if len(csets) > 0:
            ...
            med_data.loc[med_data.index.isin(csets), 'Cricality'] = f'CSets_{amount_csets}'
            csets = []
    return med_data


def state_estimation(path = '.', line_params = 'Dados_linha_XIV_barras.xlsx', meansured_file = 'Meansured_data_XIV_bus.xlsx', meansured_data_instante = None):
    y_bar_matriz, network_values = mount_y_bar(path, line_params)
    meansured, R, order_meansured = read_meansured(path, meansured_file, data= meansured_data_instante)
    _, E, med_data = linear_state_estimation(order_meansured, y_bar_matriz)
    ...
    # print('-----------------------------------')
    State_dataframe = pd.DataFrame(state_array[n_bus-1:], columns = ['Voltage Mag.'], index = list(range(1,n_bus+1)))
    State_dataframe['Angle [Deg]'] = np.degrees([0]+state_array[:n_bus-1].tolist())
    Error_SE, zArray = measurement_Function(order_meansured, meansured, y_bar_matriz, network_values, state_array)
    data_SE = pd.DataFrame([med.keys() for med in order_meansured], columns = ['Meansured Type'])
    data_SE['Location'] = [tuple(loc.values())[0] for loc in order_meansured]
    data_SE['Meansured'] = [meansured[list(x.keys())[0].split('_')[0]][list(x.values())[0]][0] for x in order_meansured]
    data_SE['Meansured Estimated'] = zArray
    data_SE['Error'] = np.where(np.abs(Error_SE) < 1e-6, 0, Error_SE)
    Sigma = np.linalg.inv(g_matrix)
    Erii = np.abs(R - h_matrix@Sigma@H_t)
    Erii = np.sqrt(np.diag(Erii))
    data_SE['Error_Normalize'] = np.where(Erii > 0, np.abs(data_SE['Error']/Erii), 0)
    ...
    return y_bar_matriz, criticality_data, J_list, J_critical, State_dataframe, data_SE
